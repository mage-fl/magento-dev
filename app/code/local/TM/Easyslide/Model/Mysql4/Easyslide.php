...
class TM_Easyslide_Model_Mysql4_Easyslide extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    {
        $getSlidersData = $this->_getReadAdapter()->select()
            ->from(array('ess' => $this->getTable('easyslide/easyslide')), array('identifier','title'))
            ->where('ess.status = ?', 1);

        return $this->_getReadAdapter()->fetchAll($getSlidersData);
    }
}